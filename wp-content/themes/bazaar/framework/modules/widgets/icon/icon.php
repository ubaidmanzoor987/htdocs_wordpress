...
class BazaarQodefIconWidget extends BazaarQodefWidget {
	public function __construct() {
		parent::__construct(
			'qodef_icon_widget',
			esc_html__( 'Select Icon Widget', 'bazaar' ),
			array( 'description' => esc_html__( 'Add icons to widget areas', 'bazaar' ) )
		);
	...
	protected function setParams() {
		$this->params = array_merge(
			bazaar_qodef_icon_collections()->getIconWidgetParamsArray(),
			array(
				array(
					'type'  => 'textfield',
					'name'  => 'icon_text',
					'title' => esc_html__( 'Icon Text', 'bazaar' )
				),
				array(
					'type'  => 'textfield',
					'name'  => 'link',
				...
				array(
					...
					'options' => array(
						'_self'  => esc_html__( 'Same Window', 'bazaar' ),
						'_blank' => esc_html__( 'New Window', 'bazaar' )
					)
				),
				array(
					'type'  => 'textfield',
	...
	public function widget( $args, $instance ) {
		...
		$data   = array();
		$data[] = ! empty( $instance['icon_hover_color'] ) ? bazaar_qodef_get_inline_attr( $instance['icon_hover_color'], 'data-hover-color' ) : '';
		$data   = implode( ' ', $data );
		...
		if ( ! empty( $instance['icon_size'] ) ) {
			$holder_styles[] = 'font-size: ' . bazaar_qodef_filter_px( $instance['icon_size'] ) . 'px';
		}
		...
		<a <?php bazaar_qodef_class_attribute( $holder_classes ); ?> <?php echo wp_kses_post( $data ); ?> href="<?php echo esc_url( $link ); ?>" target="<?php echo esc_attr( $target ); ?>" <?php echo bazaar_qodef_get_inline_style( $holder_styles ); ?>>
