...
if ( ! function_exists( 'bazaar_qodef_register_header_vertical_type' ) ) {
	/**
	 * This function is used to register header type class for header factory file
	 */
	function bazaar_qodef_register_header_vertical_type( $header_types ) {
		$header_type = array(
			'header-vertical' => 'BazaarQodef\Modules\Header\Types\HeaderVertical'
		);
...
if ( ! function_exists( 'bazaar_qodef_init_register_header_vertical_type' ) ) {
	/**
	 * This function is used to wait header-function.php file to init header object and then to init hook registration function above
	 */
	function bazaar_qodef_init_register_header_vertical_type() {
		add_filter( 'bazaar_qodef_register_header_type_class', 'bazaar_qodef_register_header_vertical_type' );
	}
	
	add_action( 'bazaar_qodef_before_header_function_init', 'bazaar_qodef_init_register_header_vertical_type' );
}

if ( ! function_exists( 'bazaar_qodef_include_header_vertical_menu' ) ) {
	/**
	 * Registers additional menu navigation for theme
	 */
	function bazaar_qodef_include_header_vertical_menu( $menus ) {
		$menus['vertical-navigation'] = esc_html__( 'Vertical Navigation', 'bazaar' );
		
		return $menus;
	}
	
	if ( bazaar_qodef_check_is_header_type_enabled( 'header-vertical' ) ) {
		add_filter( 'bazaar_qodef_register_headers_menu', 'bazaar_qodef_include_header_vertical_menu' );
	}
}

if ( ! function_exists( 'bazaar_qodef_register_header_vertical_widget_areas' ) ) {
	/**
	 * Registers additional widget areas for this header type
	 */
	function bazaar_qodef_register_header_vertical_widget_areas() {
		register_sidebar(
			array(
				'id'            => 'qodef-vertical-area',
				'name'          => esc_html__( 'Header Vertical Widget Area', 'bazaar' ),
	...
	if ( bazaar_qodef_check_is_header_type_enabled( 'header-vertical' ) ) {
		add_action( 'widgets_init', 'bazaar_qodef_register_header_vertical_widget_areas' );
	}
}

if ( ! function_exists( 'bazaar_qodef_get_header_vertical_widget_areas' ) ) {
	/**
	 * Loads header widgets area HTML
	 */
	function bazaar_qodef_get_header_vertical_widget_areas() {
...
if ( ! function_exists( 'bazaar_qodef_get_header_vertical_main_menu' ) ) {
	/**
	 * Loads vertical menu HTML
	 */
	function bazaar_qodef_get_header_vertical_main_menu() {
		bazaar_qodef_get_module_template_part( 'templates/vertical-navigation', 'header/types/header-vertical' );
	}
}

if ( ! function_exists( 'bazaar_qodef_vertical_header_holder_class' ) ) {
	/**
	 * Return holder class
	 */
	function bazaar_qodef_vertical_header_holder_class() {
...
if ( ! function_exists( 'bazaar_qodef_header_vertical_per_page_custom_styles' ) ) {
	/**
	 * Return header per page styles
	 */
	function bazaar_qodef_header_vertical_per_page_custom_styles( $style, $class_prefix, $page_id ) {
	...
	add_filter( 'bazaar_qodef_add_header_page_custom_style', 'bazaar_qodef_header_vertical_per_page_custom_styles', 10, 3 );
}