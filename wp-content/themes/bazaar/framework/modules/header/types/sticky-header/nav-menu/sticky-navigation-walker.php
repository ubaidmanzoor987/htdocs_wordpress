...
if (!class_exists('BazaarQodefStickyNavigationWalker')) {
	class BazaarQodefStickyNavigationWalker extends Walker_Nav_Menu {
		...
		function start_lvl( &$output, $depth = 0, $args = array() ) {
            ...
            if($depth == 0){
                $out_div = '<div class="second '.$background_image_class.'"><div class="inner" '.($menu_image_background != '' ? bazaar_qodef_get_inline_style($menu_image_background) : '').'>';
            }else{
                $out_div = '';
            }

            // build html
            $output .= "\n" . $indent . $out_div  .'<ul '.($menu_image_background != '' ? bazaar_qodef_get_inline_style($menu_image_background) : '').'>' . "\n";
		}
